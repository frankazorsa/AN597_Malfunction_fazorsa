   ...
   #p.val <- NULL
      ...
      SE.phat <- NULL
        ...
        SE.phat <- sqrt(phat*(1-phat)/n1)# standar error of the sample proportion
         ...
         lower <- phat - qnorm(conf.level)*SEphat
         upper <- phat + qnorm(conf.level)*SEphat
         ci <- c(lower, upper)         }
...
***
###Challenges first question: I am still testing if this function works or not.
***
***
###Second question:
...
pi <- predict(lm_fit, newdata = data.frame(Brain_Size_Species_Mean = d$Brain_Size_Species_Mean), interval = "prediction", 
...
g <- g + geom_line(aes(x = x, y = CIlwr, colour = "Confidence Interval"))
g <- g + geom_line(aes(x = x, y = CIupr, colour = "Confidence Interval"))
g <- g + geom_line(data = pf, aes(x = x, y = PIlwr, colour = "Prediction Interval"))
g <- g + geom_line(data = pf, aes(x = x, y = PIupr, colour = "Prediction Interval"))
g <- g + scale_colour_manual("", values = c("black"="black", "Confidence Interval"="blue", "Prediction Interval"="red"))
g <- g + theme (
...
```
***
###Challenges second question: I could not complete the last part of the second question, but I will keep working on it. 
***
